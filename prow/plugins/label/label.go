...
var (
	labelRegex              = regexp.MustCompile(`(?m)^/(area|committee|kind|priority|sig|branch|queue|version|scrum|status|from|rc|rt|type|product|need|infra-area|resolution|rca)\s*(.*)$`)
	removeLabelRegex        = regexp.MustCompile(`(?m)^/remove-(area|committee|kind|priority|sig|branch|queue|version|scrum|status|from|rc|rt|type|product|need|infra-area|resolution|rca)\s*(.*)$`)
	singleChoice            = flag.String("single-choice", "area,kind,priority,queue,scrum,status,from,rc,rt,type,product,resolution", "Comma separated list of command that needs support single-choice")
