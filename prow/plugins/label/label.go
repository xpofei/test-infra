...
var (
	labelRegex              = regexp.MustCompile(`(?m)^/(area|committee|kind|priority|sig|branch|queue|version|scrum|status|from|rca|rc|rt|type|product|need|infra-area|resolution)\s*(.*)$`)
	removeLabelRegex        = regexp.MustCompile(`(?m)^/remove-(area|committee|kind|priority|sig|branch|queue|version|scrum|status|from|rca|rc|rt|type|product|need|infra-area|resolution)\s*(.*)$`)
